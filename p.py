...
async def main():
    ...
    for sess in sessions_data:
        ...
        register_handlers(abh_dict)
        ABHs.append(abh_dict)

    # تشغيل جميع العملاء بالتوازي
    await asyncio.gather(*[abh["client"].start() for abh in ABHs])
    await asyncio.gather(*[abh["client"].run_until_disconnected() for abh in ABHs])
