...
banned_words = ["سب"]  # إضافة الكلمات المحظورة

def normalize_text(text):
    """إزالة كل شيء عدا الأحرف مع إزالة الأرقام والحروف 'ـ' و 'ى'"""
    # إزالة أي شيء غير حرف أو الحروف 'ـ' و 'ى' و الأرقام
...
async def handler(event):
    ...
    # التحقق من الرسالة الأصلية
        ...
        await event.reply(warning_msg)  # إرسال رسالة تحذير للمستخدم

@ABH.on(events.MessageEdited)
async def handler_edited(event):
    """التعامل مع الرسائل المعدلة"""
    if check_message(event.text):  # التحقق من الرسالة المعدلة
        user_id = event.sender_id
        warning_msg = "🚨 **تحذير:** لا يمكنك استخدام كلمات محظورة في المحادثة! 🚫"
        await event.reply(warning_msg)  # إرسال رسالة تحذير للمستخدم
