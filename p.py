...
async def a1(event):
    user_states[event.sender_id] = {'step': 'get_subject1'}
    await event.edit("أرسل الموضوع (الكليشة القصيرة)")
@client.on(events.NewMessage)
async def handle_message(event):
    global isInfo
    ...
    state = user_states[user_id]
    step = state['step']
    if step == 'get_subject1':
        state['subject1'] = event.text
        state['step'] = 'get_body1'
        await event.respond("أرسل نص الكليشة (الكليشة الكبيرة)")
    elif step == 'get_body1':
        state['body1'] = event.text
        state['step'] = 'get_recipient1'
        await event.respond("أرسل الإيميل المستلم (`abuse@telegram.org`)")
    elif step == 'get_recipient1':
        state['recipient1'] = event.text
        state['step'] = 'get_email1'
        await event.respond("أرسل بريدك الإلكتروني (الايميل الذي تريد منه الارسال)")
    elif step == 'get_email1':
        state['sender_email'] = event.text
        state['step'] = 'get_password1'
        await event.respond("أرسل كلمة المرور (كلمة مرور التطبيق كما في الفديو)")
    elif step == 'get_password1':
        state['password1'] = event.text
        subject1 = state.get('subject1')
        body1 = state.get('body1')
        recipient1 = state.get('recipient1')
        sender_email1 = state.get('sender_email')
        password1 = state.get('password1')
        if not all([subject1, body1, recipient1, sender_email1, password1]):
            await event.respond("حدث خطأ أثناء جمع البيانات. يرجى المحاولة مرة أخرى.")
            isInfo = False
        else:
            isInfo = True
        email_message = create_email_message(subject1, body1, recipient1)
        buttons = [
            [Button.inline("إرسال الرسالة", b"send_email")]
        ]
        await event.respond(
...
async def send_email(event):
    ...
    if user_id not in user_states or user_states[user_id].get('step') != 'confirm_send':
        await event.edit("أحدا أو كل المعلومات فيها نقص. \n حاول مره أخرى مع /start")
        return
    state1 = user_states[user_id]
    subject1 = state['subject1']
    body1 = state['body1']
    recipient1 = state['recipient1']
    sender_email1 = state['sender_email']
    password1 = state['password1']
    try:
        message = MIMEMultipart("alternative")
        message["Subject1"] = subject1
        ...
        with smtplib.SMTP_SSL(default_smtp_server, default_smtp_port) as server:
            ...
            for i in range(100):
                server.sendmail(sender_email1, recipient1, message.as_string())
                if i == 0:
                    await event.edit(f"تم الإرسال {i+1} بنجاح")
                else:
                    await event.respond(f"تم الإرسال {i+1} بنجاح")
