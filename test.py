...
async def start_game(message):
    ...
    await delete_message_after(message.chat.id, sent_msg.message_id, delay=3)

async def delete_message_after(chat_id, message_id, delay=3):
...
async def handle_start_game(call):
    ...
    if not group_game_status[chat_id]['game_active']:
        ...
        await delete_message_after(chat_id, sent_msg2.message_id, delay=3)
        ...
        try:
            ...
            if 1 <= guess <= 6:
                if guess == number2:
                    ...
                    await delete_message_after(message.chat.id, sent_msg3.message_id, delay=3)
                    reset_game(chat_id)
                else:
                    sender_first_name = message.from_user.first_name
                    game_board = [["❌" if i == guess - 1 else "🖐️" for i in range(6)]]
                    sent_msg4 = bot.reply_to(message, f"ضاع البات ماضن بعد تلگونة ☹️ \n{format_board(game_board, numbers_board)}")
                    await delete_message_after(message.chat.id, sent_msg4.message_id, delay=3)
                    reset_game(chat_id)
            else:
                sent_msg5 = bot.reply_to(message, "❗ يرجى إدخال رقم صحيح بين 1 و 6.")
                await delete_message_after(message.chat.id, sent_msg5.message_id, delay=3)
        except (IndexError, ValueError):
            sent_msg6 = bot.reply_to(message, "❗ يرجى إدخال رقم صحيح بين 1 و 6.")
            await delete_message_after(message.chat.id, sent_msg6.message_id, delay=3)
...
async def show_number(message):
    ...
    if chat_id in group_game_status and group_game_status[chat_id]['game_active']:
        ...
        await delete_message_after(message.chat.id, sent_msg9.message_id, delay=3)
        
        sent_msg10 = bot.reply_to(message, "تم إرسال الرقم السري إلى @k_4x1.")
        await delete_message_after(message.chat.id, sent_msg10.message_id, delay=3)
    else:
        sent_msg11 = bot.reply_to(message, "لم تبدأ اللعبة بعد. أرسل 'محيبس' لبدء اللعبة.")
        await delete_message_after(message.chat.id, sent_msg11.message_id, delay=3)
